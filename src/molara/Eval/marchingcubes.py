...
def marching_cubes(
    ...
    # num_unique_edges = (
    #     3 * x_voxels * y_voxels * z_voxels
    #     - x_voxels * y_voxels
    #     - x_voxels * z_voxels
    #     - y_voxels * z_voxels
    # )
    num_edges_y_z = 2 * y_voxels * z_voxels - y_voxels - z_voxels
    verts1 = []
    verts2 = []
            ...
            for k in range(z_voxels - 1):
                ...
                for ei in range(5):
                    if edges[ei * 3] == -1:
                        ...
                        t1 = calculate_interpolation_value(isovalue, v11, v12)
                        t2 = calculate_interpolation_value(isovalue, v21, v22)
                        t3 = calculate_interpolation_value(isovalue, v31, v32)
                        ...
                        if i < x_voxels - 2 and j < y_voxels - 2 and k < z_voxels - 2:
                            n1 = list(-calculate_normal_vertex(grid, voxel_indices[c11, :], voxel_indices[c12, :], t1))
                            n2 = list(-calculate_normal_vertex(grid, voxel_indices[c21, :], voxel_indices[c22, :], t2))
                            n3 = list(-calculate_normal_vertex(grid, voxel_indices[c31, :], voxel_indices[c32, :], t3))
                        else:
                            n = np.cross(
                        ...
                        verts1 += vertex1 + n1 + vertex2 + n2 + vertex3 + n3
                phase = -1
                edges = get_edges(isovalue, voxel_values, phase=phase)
                for ei in range(5):
                    if edges[ei * 3] == -1:
                        break
                    else:
                        ...
                        t1 = calculate_interpolation_value(isovalue * phase, v11, v12)
                        t2 = calculate_interpolation_value(isovalue * phase, v21, v22)
                        t3 = calculate_interpolation_value(isovalue * phase, v31, v32)
                        ...
                        if i < x_voxels - 2 and j < y_voxels - 2 and k < z_voxels - 2:
                            ...
                            n1 = n2 = n3 = list(n / np.linalg.norm(n))
                        verts2 += vertex1 + n1 + vertex2 + n2 + vertex3 + n3
