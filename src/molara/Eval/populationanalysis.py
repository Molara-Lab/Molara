...
class PopulationAnalysis:
    ...
    def __init__(self, molecule: Molecule):
        ...
        if self.mo_type == 'cartesian':
            ...
            self.d_matrix = np.dot(self.mo_coefficients[:, :n_occ], self.mo_coefficients[:, :n_occ].T) * 2
            ...
            for i, basis_function_i in enumerate(self.basis_functions):
                for j, basis_function_j in enumerate(self.basis_functions):
                    self.overlap_matrix_ao[i, j] = contracted_overlap(basis_function_j,
                                                                      ...
                                                                      self.basis_functions_positions[j],
                                                                      self.basis_functions_positions[i])
            self.overlap_matrix_ao_spherical = np.dot(self.transformation_matrix,
                                                      np.dot(self.overlap_matrix_ao, self.transformation_matrix.T))
            self.overlap_matrix_mo = np.dot(self.mo_coefficients_spherical.T,
                                           np.dot(self.overlap_matrix_ao_spherical, self.mo_coefficients_spherical))
            n_occ = int(self.number_of_electrons / 2)
            self.d_matrix = np.dot(self.mo_coefficients_spherical[:, :n_occ], self.mo_coefficients_spherical[:, :n_occ].T) * 2
