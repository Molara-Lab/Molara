...
class TestMolecule(TestCase):
    ...
    def test_center_coordinates(self) -> None:
        ...
        _assert_array_almost_equal(self.ccl4.center_of_mass, np.zeros(3))
        offset = np.array([1.234, 2.345, 3.456])
        for atom_i in self.ccl4.atoms:
            atom_i.set_position(atom_i.position + offset)
        self.ccl4.center_coordinates()
        _assert_array_almost_equal(self.ccl4.center_of_mass, np.zeros(3))
        _assert_array_almost_equal(self.ccl4.center, offset)

        # test water
        self.water.center_coordinates()
        _assert_array_almost_equal(self.water.center_of_mass, np.zeros(3))
        offset = np.array([1.456, 2.567, 3.678])
        for atom_i in self.water.atoms:
            atom_i.set_position(atom_i.position + offset)
        self.water.center_coordinates()
        _assert_array_almost_equal(self.water.center_of_mass, np.zeros(3))
        _assert_array_almost_equal(self.water.center, offset)
